...
const defineTerrain = function () {
  ...
  for (let i = 0; i < game.terrainArray.length; i++) {
      ctx.bezierCurveTo(game.terrainArray[i][0] - game.terrainArray[i][2], game.terrainArray[i][1] - game.terrainArray[i][3], game.terrainArray[i][0] + game.terrainArray[i][2], game.terrainArray[i][1] + game.terrainArray[i][3],game.terrainArray[i][0], game.terrainArray[i][1]);
  }
  // connect polygon
  ...
  switch (e.target.id) {
    ...
    case 'fire-button':
      ...
      let currentTank = game.tankObjects[game.currentPlayerIdx];
          ...
          currentTank.fire();
          setTimeout(function () {
              const explSound = document.getElementById('explosion-sound');
              if (explSound) {
                  explSound.currentTime = 0;
                  explSound.play();
              };
              showDialog()
          }, Math.sin(degreesToRadians(game.tankObjects[game.currentPlayerIdx].turret.angle)) * EXPLOSION_DELAY); // 20
